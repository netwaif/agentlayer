---
name: orchestration
description: AgentLayer로 멀티 에이전트 오케스트레이션. worker N개(claude·codex·gemini)를 각각 git worktree+tmux window에 띄워 태스크를 dispatch하고, 완료를 기다렸다가 결과를 취합·비교한다. "worker 2개 만들어서 같은 태스크 시켜줘", "claude랑 codex한테 A/B로 시켜봐", "서로 다른 태스크 나눠서 병렬로", "이 작업 worktree에서 codex한테 시켜"(worker 1개도 같은 절차), "/orchestration" 등으로 트리거. 머지는 기본적으로 사용자 몫이다. 브라우저 조작·스크린샷·지목·쿠키는 이 스킬이 아니라 agent-browser 스킬이다.
---
...
- 브라우저가 필요한 일(화면 확인·스크린샷·콘솔 진단·지목·쿠키)은 **agent-browser 스킬**의 규칙을 따른다 — chrome-devtools MCP, 자기 탭만. worker에게 브라우저 일을 시킬 때도 지시문에 "자기 탭에서만"을 넣는다.
- worker가 dev 서버를 띄우면 `http://localhost:<포트>`를 한 줄로 찍게 한다. 사용자는 관제탑 `p`나 터미널 링크 ⌘-클릭으로 같은 브라우저에서 본다. 비교용 서버는 4절대로 코디네이터가 worktree를 cwd로 띄운다.
