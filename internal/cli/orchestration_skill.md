...
- 사용자가 화면을 봐야 하는데 원격(SSH·디스코드에서 온 지시)이거나 "스크린샷 폰으로 보내줘"라고 하면 `agentlayer browser shot <url> --notify`로 그 페이지 스크린샷을 알림 웹훅(폰 Discord)에 보낸다. 봇 세션처럼 PATH가 최소면 `~/.local/bin/agentlayer`로 부른다.
