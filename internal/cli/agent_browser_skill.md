...
- 도구를 부르면 그 탭이 **작업 탭**이 되어 어둡게 덮이고 AI 커서(동작 라벨)·하단 알약이 뜬다. 다른 탭에는 얇은 띠만 뜬다("AI가 다른 탭에서 작업 중 · <제목>"). 20초 동안 호출이 없으면 저절로 내려간다.
- 사용자가 알약의 「내가 조작하기」를 누르면 다음 도구 호출이 **잡혀서 기다린다**(기본 120초, 설정 `browser_control_wait_seconds`). 「AI에게 돌려주기」를 누르면 이어서 실행된다. 기다림은 프록시가 하므로 에이전트는 아무것도 안 해도 된다.
- 「중단」을 누르면 도구 호출에 "사용자가 브라우저 조작을 중단시켰습니다 — 재시도하지 말고 사용자 지시를 기다리세요"가 돌아오고, 대기 시간을 넘기면 "사용자가 브라우저를 직접 조작 중입니다 — … 재시도하지 말고 사용자에게 알린 뒤 지시를 기다리세요"가 돌아온다. 둘 다 **재시도하지 말고** 사용자에게 한 줄로 알린 뒤 지시를 기다린다.
- 알약·버튼은 기동 시 프로필에 붙는 확장이 그린다 — **이미 떠 있는 브라우저에는 다음 기동부터** 보인다. 확장이 안 붙은 브라우저에서 사용자 소유로 잠겨 호출이 계속 막히면(중단 문구가 반복되면) 사용자에게 `agentlayer browser control reset`을 안내한다(제어권을 idle로 되돌린다).
- `new_page`는 앞 탭으로 열린다 — 사용자가 브라우저 창을 보고 있으면 작업이 그대로 보여야 한다(촬영·감시). 배경 탭은 그리기가 멈춰 `click`이 "did not become interactive"로 줄줄이 실패하므로 `background: true`를 스스로 붙이지 않는다(설정 `browser_background_tabs`로만 켠다). 기동 직후에는 원래 앞에 있던 앱으로 포커스가 되돌아가므로 앱이 앞으로 튀어나오진 않는다(macOS). 사용자가 화면을 보게 하려면 URL을 한 줄로 찍어 준다.

## 호출 효율

- 여러 단계로 데이터를 뽑을 땐 `evaluate_script` 하나 안에서 기다렸다가(폴링) 추출한다. `wait_for`·`navigate_page` 응답의 페이지 스냅샷은 잘려서 오므로(토큰 절약, `browser_trim_snapshots` 기본 true) 구조가 필요하면 `take_snapshot`을 따로 부른다.
...
### 타임아웃이 났을 때 — 브라우저를 직접 재시작하지 않는다

- `did not become interactive within the configured timeout`(click)은 대개 **페이지가 느린 것**이다(무거운 예매·쇼핑 SPA에서 흔함). 굳음의 표식이 아니다. 5초쯤 뒤 같은 호출을 한 번 다시 하고, 안 되면 `take_snapshot`으로 현재 상태를 읽고 다음 동작을 고른다.
- `Page.captureScreenshot timed out`(screenshot)은 페이지가 느린 것일 수도, 브라우저가 화면 프레임을 못 내는 상태일 수도 있다. 프록시가 그 에러 줄에 `[agentlayer] …` 설명을 붙인다. 5초 뒤 한 번만 재시도하고, 그래도 같으면 **사용자에게 알리고 멈춘다.**
- **`agentlayer browser restart`를 스스로 실행하지 않는다.** 재시작은 다른 세션·사용자가 쓰는 탭까지 전부 날린다(2026-09-24: 촬영 중 브라우저가 8번 재시작된 사고). 굳음의 판정과 재시작은 행 감시(훅)가 프레임 프로브를 3회 연속 확인한 뒤에만 한다. 사용자가 재시작하라고 말했을 때만 실행한다.
- 재시작 뒤 같은 증상이면 연결 문제다: 사용자에게 알린다(`/mcp`에서 chrome-devtools 재연결 또는 세션 재시작은 사용자 몫).
