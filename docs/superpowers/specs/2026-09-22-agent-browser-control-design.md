...
- **프레임 프로브(2026-09-23 추가)**: 실측된 "창 전체 굳음"은 UI 스레드 행이 아니었다 — GPU 프로세스의 vsync 시계(CVDisplayLink 스레드)가 사라져 프레임이 한 장도 안 나오는 상태(CDP·JS 즉답, 탭 목록 정상, 새 창도 안 그려짐, 트레이스에 BeginFrame 0건). 위 ping은 4시간 동안 통과했다. 그래서 ping 뒤에 보이는 탭(`document.visibilityState=visible`, 웹 탭 우선) 하나에서 8×8 `Page.captureScreenshot`이 같은 예산 안에 오는지를 본다. 숨은 탭은 원래 프레임이 없으므로 제외, 보이는 탭이 없으면 판정 생략. 실패 횟수·재기동은 기존 3회 규칙 그대로. 사람 쪽 비상구는 `agentlayer browser restart`(Kill→Relaunch). 프록시는 screenshot·click 타임아웃 에러 줄에 복구 힌트를 덧붙인다(`browser/framehint.go`). Chrome 내부 트리거는 미상 — 디스플레이 꺼짐(기동 중·4초·90초)·SIGSTOP(브라우저·GPU, 12초·60초)·vsync 플래그로는 재현 안 됨. 기동 플래그에 `--enable-logging --v=0 --vmodule=display_link_mac=2,external_begin_frame_source_mac=2`를 더해 다음 재발 때 프로필 `chrome_debug.log`에 증거가 남게 한다.
