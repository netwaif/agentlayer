...
- `preview_auto`(선택): 에이전트 폴더 아래에 새 dev 서버가 뜨면 전용 브라우저에
  자동으로 연다. 기본 `true`. 같은 서버는 한 번만, 이미 탭이 있으면 안 연다.
...
- **관제탑에서 키 하나로**: 에이전트 행을 고르고 `b`(요소 지목)·`s`(활성 탭
  ...
  폴더 아래에서 dev 서버가 listen 중이면 행 끝에 `🌐:3000` 뱃지가 붙고, 처음
  보는 서버는 자동으로 전용 브라우저 창(⎇브랜치 제목)에 열린다(`preview_auto`).
  `p`는 그 서버를 다시 연다. 위 명령들은 이 키들이 뒤에서 부르는 배관이다
