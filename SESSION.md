...
**GitHub 릴리즈 latest = v1.4.0**(57a407f, 2026-09-08 새벽 정식, tap Casks 1.4.0, install.sh latest 조회 v1.4.0). 로컬 main = 57a407f = origin/main. make install = v1.4.0. WSL2 rc2 재검증(NAS `RESULT-wsl2-rc2-20260908.md`) agentlayer 4건 전부 ✓ → 정식 릴리즈함. 남은 1건(chat-claude MCP 신뢰 프롬프트)은 **설치기 0.1.18**(28fa323, 푸시됨)로 수정 — 맥 재현·단위 테스트만, WSL2 실기 재확인 대기(NAS `README-agentlayer-wsl2-followup-0.1.18.md`). 매뉴얼 v1.3.1 Drive 배포 완료. 디스코드 공지(v1.3.x+v1.4.0 묶음) **초안만** 있음. VM `ubuntu-agent` 일시정지.

## 다음 단계
<!-- 덮어쓰기. 첫 항목 = 다음 세션이 바로 집어들 일 -->

1. **디스코드 멤버 공지**(사용자 확인 뒤 게시) — 초안 scratchpad `discord-announce-v1.4.0.md`(v1.3.x 브라우저·cookies export·FX + v1.4.0 리눅스/WSL2 + apt 한 줄 + 릴리즈 노트 링크). 초안이 scratchpad에만 있으니 세션이 바뀌었으면 릴리즈 노트(`gh release view v1.4.0`)로 다시 뽑는다. 리눅스 매뉴얼 절: 릴리즈 노트 "리눅스에서 다른 점" 5줄이 전부라 매뉴얼 개정 대신 공지·README로 충분하다고 판단 — 사용자 결정 대기.
2. **WSL2 설치기 0.1.18 재확인 결과 읽기** — NAS `/Volumes/private/mac-to-win10/RESULT-wsl2-followup-0.1.18-*.md`(Win10 세션이 `README-agentlayer-wsl2-followup-0.1.18.md` 절차대로). 확인 항목: install 출력 `폴더 신뢰 선등록 … /chat` 줄, chat-claude pane에 "New MCP server found" 없이 입력창, remove 출력 `폴더 신뢰 선등록 회수`. 실패하면 `~/.claude.json` projects 키 경로(resolve된 절대경로)부터 대조.
3. 후속 후보: `internal/discord/card.go`·`card_test.go` gofmt 미적용(기존, 내용 무관) / agy 권한 프롬프트 대기가 [WORK]로 보임(agy 훅에 승인 대기 이벤트 없음 — 표시 개선 여지) / 에이전트 브라우저 창 식별 강화 / FX iframe / mcp-serve resize 차단 / CfT 갱신 명령 / favicon 404 소음 / autopreview 포트 제외 / 관제탑 restore 체크리스트 키 / codex trusted_hash 자동 기록 / `agentlayer browser`(인자 없음) 재실행 시 붙어 있는 동작·`browser errors` Enter 대기.
