...
**GitHub 릴리즈 = v1.2.5**. `browser-mcp`는 main에 fast-forward 머지 완료(로컬 main = ee1fa6f, origin 대비 70커밋 앞, 미푸시). 2026-09-04 커밋 2건: c1c95e0 에이전트 브라우저 엔진 Chrome for Testing 전환(+플래그 test-type=gpu·site-isolation 제거·번역 끔), ee1fa6f Ask 필드 분리(DONE 행 "needs your permission" 잔존 버그). make install = ee1fa6f. 에이전트 브라우저는 CfT 152.0.7977.82로 떠 있음(9222). v1.3.0 릴리즈는 사용자 "재정박하고 대기" 지시로 **대기 중**(로컬 준비 완료). 릴리즈 노트 초안은 scratchpad에 있었음(세션 종료로 소실 가능 — 다음 단계 1 목록으로 재작성). 워킹트리 클린.

## 다음 단계
<!-- 덮어쓰기. 첫 항목 = 다음 세션이 바로 집어들 일 -->

1. **v1.3.0 릴리즈**(사용자 "릴리즈 해" 신호 뒤): `git push origin main browser-mcp` → `git tag v1.3.0` → push 태그 → `GITHUB_TOKEN=$(gh auth token) goreleaser release --clean --release-notes <파일>`(SESSION.md 더티면 stash 먼저) → tap Casks/agentlayer.rb 버전 확인(goreleaser 자동 커밋) → 디스코드 공지 → `/deploy-manual agentlayer`(VERSION 1.2.0→1.3.0). 릴리즈 노트 절: 에이전트 브라우저(전용 인스턴스·pick/--once·shot --send/--notify·errors --reload·preview ⎇탭·cookies import/list/clear·open+iTerm2·mcp-serve 지연 기동·MCP roots) / 관제탑(b·s·p·🌐·팝업 크기) / worktree·worker(restore 체크리스트·봇 중복 방지·신뢰 자동 승인·worker_auto_approve·codex hooks·SendText 지연) / 스킬 / 안정성(Capturing 잠금·usage SWR·cbreak) / **엔진 Chrome for Testing(Dock 충돌 해결, 첫 기동 200MB)·Ask 필드**. 업그레이드 안내: `brew upgrade netwaif/tap/agentlayer && agentlayer init` + 살아 있는 세션 재시작 + codex `/hooks` t.
2. 매뉴얼 원본 재부팅 절차 갱신("봇 자동 기동 → `agentlayer restore`(체크리스트) → wake-all", --dry-run 먼저 문구 제거) + 에이전트 브라우저 엔진 CfT 문단(첫 기동 다운로드·Dock 문제 설명).
3. 후속 후보: CfT 엔진 갱신 명령(`browser engine --update`, 지금은 폴더 삭제로 재다운로드), Task 칸에 wt 메타 작업 이름 표시, 관제탑 TUI restore 체크리스트 키, codex trusted_hash 자동 기록(해시 방식 미상), gemini(agy) AGENTS/GEMINI.md 브라우저 블록, favicon 404 소음, autopreview 포트 제외 옵션.
4. 촬영 뒤 정리: 8080 python 서버 종료, `~/.local/state/agentlayer/picks/` 정리, demo 저장소 `git reset --hard f1defa2`, worktree 3개(hero-bold-*) 정리.
5. 대시보드 채널 옛 핑 메시지 Discord 수동 삭제(567c2c9).
6. 보류 아이디어: `agentlayer status --prune`, Termius 컴팩트 모드, MultiAgent 패널 날짜 필터, 미리보기 원본색(-e), Orca 대비 메모리 측정, provider 게이지 텍스트화, 브라우저 프로필 다중화, `init --iterm2`.
