...
**GitHub 릴리즈 = v1.2.5**. 로컬 main = 8afde8f(origin 대비 74커밋 앞, 미푸시). 2026-09-05 커밋 3건: 62c1abf 에이전트 브라우저 조작 효과(FX: AI 커서·리플·테두리 글로우·입력 하이라이트), d189ba4 `agent-browser` 스킬 분리(orchestration은 코디네이터 절차만), 8afde8f FX 가시성(모든 도구 호출에 켜고 2.5초 유지·"AI 조작 중" 알약)+창 크기 변경 금지 규칙. make install = 8afde8f, init 재실행됨(스킬 2개·codex AGENTS 블록 갱신). 에이전트 브라우저는 새 확장으로 재기동돼 떠 있음(9222). v1.3.0 릴리즈는 여전히 사용자 신호 대기. 워킹트리 클린(SESSION.md 제외).

## 다음 단계
<!-- 덮어쓰기. 첫 항목 = 다음 세션이 바로 집어들 일 -->

1. **FX 실사용 확인**(사용자): 새 Claude 세션에서 "에이전트 브라우저로 <사이트> 조작하면서 분석해줘" → 테두리 글로우·"AI 조작 중" 알약·AI 커서가 유지되며 보이는지, 창 크기가 안 바뀌는지. 색·크기·유지시간(content.js `LINGER` 2500ms) 조정 요청 반영.
2. **v1.3.0 릴리즈**(사용자 "릴리즈 해" 신호 뒤): `git push origin main browser-mcp` → `git tag v1.3.0` → push 태그 → `GITHUB_TOKEN=$(gh auth token) goreleaser release --clean --release-notes <파일>`(SESSION.md 더티면 stash 먼저) → tap Casks/agentlayer.rb 버전 확인(goreleaser 자동 커밋) → 디스코드 공지 → `/deploy-manual agentlayer`(VERSION 1.2.0→1.3.0). 릴리즈 노트 절: 에이전트 브라우저(전용 인스턴스·pick/--once·shot --send/--notify·errors --reload·preview ⎇탭·cookies import/list/clear·open+iTerm2·mcp-serve 지연 기동·MCP roots·**조작 효과 FX·browser_fx**) / 관제탑(b·s·p·🌐·팝업 크기) / worktree·worker(restore 체크리스트·봇 중복 방지·신뢰 자동 승인·worker_auto_approve·codex hooks·SendText 지연) / **스킬(orchestration·agent-browser 분리, init이 둘 설치)** / 안정성(Capturing 잠금·usage SWR·cbreak) / 엔진 Chrome for Testing(Dock 충돌 해결, 첫 기동 200MB)·Ask 필드. 업그레이드 안내: `brew upgrade netwaif/tap/agentlayer && agentlayer init` + 살아 있는 세션 재시작(mcp-serve 새 바이너리) + 에이전트 브라우저 재기동(확장 반영) + codex `/hooks` t.
3. 매뉴얼 원본 갱신: 재부팅 절차("봇 자동 기동 → `agentlayer restore`(체크리스트) → wake-all", --dry-run 먼저 문구 제거) + 에이전트 브라우저 엔진 CfT 문단(첫 기동 다운로드·Dock 문제) + FX 문단 + 스킬 2개 안내.
4. 후속 후보: FX를 iframe 안 클릭에도(all_frames, 좌표 변환), mcp-serve에서 `resize_page` 차단 옵션(스킬 규칙만으로 부족하면), gemini(agy) GEMINI.md 브라우저 블록(agent-browser 규칙 포함), CfT 엔진 갱신 명령(`browser engine --update`), Task 칸에 wt 메타 작업 이름, 관제탑 TUI restore 체크리스트 키, codex trusted_hash 자동 기록, favicon 404 소음, autopreview 포트 제외 옵션.
5. 촬영 뒤 정리: 8080 python 서버 종료, `~/.local/state/agentlayer/picks/` 정리, demo 저장소 `git reset --hard f1defa2`, worktree 3개(hero-bold-*) 정리.
6. 대시보드 채널 옛 핑 메시지 Discord 수동 삭제(567c2c9).
7. 보류 아이디어: `agentlayer status --prune`, Termius 컴팩트 모드, MultiAgent 패널 날짜 필터, 미리보기 원본색(-e), Orca 대비 메모리 측정, provider 게이지 텍스트화, 브라우저 프로필 다중화, `init --iterm2`.
